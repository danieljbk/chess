...
function movePiece(piece, startingPosition, endingPosition) {
  ...
  if (boardPiece != '.') {
    if (
      ...
      if (boardPiece == 'p' || boardPiece == 'P') {
        if (endingPosition[0] == 0 || endingPosition[0] == 7) {
          if (curPlayer == 'white') {
            curBoard[endingPosition[0]][endingPosition[1]] = 'q'
            destinationSquare.firstChild.src = getPieceImageSource('q')
          } else {
            curBoard[endingPosition[0]][endingPosition[1]] = 'Q'
            destinationSquare.firstChild.src = getPieceImageSource('Q')
          }
...
function validateMovement(startingPosition, endingPosition) {
  ...
  switch (boardPiece) {
    ...
    case 'p':
      return validatePawnMovement('white', startingPosition, endingPosition)
    case 'P':
      return validatePawnMovement('black', startingPosition, endingPosition)
  }
}
...
function validatePathIsBlocked(startingPosition, endingPosition) {
  ...
  while (squareX != endingPosition[0] || squareY != endingPosition[1]) {
    const isSquareOccupied =
      document.getElementById(`${squareX + 1}${squareY + 1}`).children.length >
      0
